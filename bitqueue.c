...
void bitqueue_put_bits(bitqueue_t *b, int len, long long bits)
{
	int i, value;

	for (i=0 ; i<len ; i++)
	{
		value = !!( bits & (1<<(len-1-i)) );
		bitqueue_put_bit(b, value);
	}
}
...
void bitqueue_patch_number(bitqueue_t *b, int pos, int len, long long number)
{
	int index, i, value;
	...
	{
		index = pos >> 3;
		value = !!( number & (1<<(len-1-i)) );
		b->bits[index] =
			(b->bits[index] & pos_mask[pos & 7])
			| (value << pos_shift[pos & 7]);
	}
